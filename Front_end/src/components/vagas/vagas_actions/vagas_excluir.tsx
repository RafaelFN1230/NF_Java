...
export default function ActionDelete({ id, nome, onEditComplete }: ActionDeleteProps) {
  ...
  function handleExcluirVaga() {
    DeletarVaga(id)
      .then((response) => {
        toast.error("Vaga removida com sucesso!", {description: `ID da vaga:   ${id} Nome da vaga:   ${nome}`, duration: 8000 })
        setIsOpen(false);
        onEditComplete()
      })
      .catch((error) => {
