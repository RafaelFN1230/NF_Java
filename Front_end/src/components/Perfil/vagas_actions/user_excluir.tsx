...
export default function ActionDelete({ employeeId, nome, onEditComplete }: ActionDeleteProps) {
  ...
  function handleExcluirFuncionario() {
    DeletarFuncionario(employeeId)
      .then((response) => {
        toast.error("Funcionário removido com sucesso!", {description: `ID do funcionário:   ${employeeId} Nome do funcionário:   ${nome}`, duration: 8000 })
        setIsOpen(false);
        onEditComplete()
      })
      .catch((error) => {
